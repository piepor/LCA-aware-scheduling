...
#            if LINEAR:
#                resultsPath = './results/raw/linear-regression/'
#                modelsPath = './models/raw/linear-regression/'
#            elif NN:
#                resultsPath = "".join(['./results/raw/nn/', str(nUnit), 'neurons']) 
#                modelsPath = "".join(['./models/raw/nn/', str(nUnit), 'neurons']) 
#            elif RNN:
#                resultsPath = "".join(['./results/raw/rnn/', str(nUnit), 'neurons']) 
#                modelsPath = "".join(['./models/raw/rnn/', str(nUnit), 'neurons']) 
            if LINEAR:
...
# scale dataframe
            ...
            for i, technology in enumerate(technologiesDF.keys()):
                # select technology
                df = technologiesDF[technology]
                x, y = seriesToSupervised(df, lagsIn=7*24, lagsForecast=24)
                #fold = 0
                #filePath = "".join([resultsPath, technology.replace(" ", "-"),'-metrics.log'])
                #with open(filePath, 'a') as file:
                #    file.write("".join([technology, " model"]))
                r2Tot = 0
                rmseTot = 0
                smapeTot = 0
                r2TotRescaled = 0
                ...
                MAX_EPOCHS = 200
                patience = 30
                filePath = os.path.join(resultsPath, "".join([technology.replace(" ", "-"),'-metrics.log']))
                try:
                    os.remove(filePath)
                except Exception:
                    pass
                ...
                if RNN:
#                    model = tf.keras.Sequential([
#                    tf.keras.layers.LSTM(units=nUnit, activation='relu', return_sequences=True, return_state=),
#                    tf.keras.layers.Dense(units=24)
#                    ])
...
#        fig.show()
                ...
                if LINEAR:
                    fileModel = os.path.join(modelsPath, "".join([technology.replace(" ", "-"), "-model.pkl"]))
                    pickle.dump(model, open(fileModel, 'wb'))
                else:
                    model.save(os.path.join(modelsPath, technology.replace(" ", "-")))
